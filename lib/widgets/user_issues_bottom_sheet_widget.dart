...
class UserIssuesBottomSheetWidget extends ConsumerStatefulWidget {
  final ChangeNotifierProvider<MainProvider> changeMainProvider;
  final ScrollController scrollController;
  const UserIssuesBottomSheetWidget({
    super.key,
  ...
  @override
  ConsumerState<UserIssuesBottomSheetWidget> createState() =>
      _UserIssuesBottomSheetWidgetState();
}

class _UserIssuesBottomSheetWidgetState
    extends ConsumerState<UserIssuesBottomSheetWidget> {
  late Timer timer;
  @override
  void initState() {
    super.initState();
  ...
  void init() async {
    issues = await ref.read(widget.changeMainProvider).getUserIssues(
          ref.read(globalProvider).user?.id ?? '',
        );
    loading = false;
    setState(() {});
  }
                              ...
                              child: Column(
                                children: issues.map((issue) {
                                  return IssueListTileWidget(
                                    ...
                                    isEditing: isEditing,
                                    onDelete: () async {
                                      mainProvider.deleteIssue(issue);
                                      issues.remove(issue);
                                      setState(() {});
                                    },
                                  );
